...
$uploadOk = 0;
            ...
            if($name!=""){
                if($id==!""){
                    ...
                    $stmt = $conn -> prepare("INSERT INTO product(name, price, discount, img, id_cate, create_at, update_at, description,deleted,status)
                    VALUES ('$name', $price, $discount, '$img', $id_cate, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '$description',0,1)");
                    $stmt->execute();
                    header('Location: admin.php?page=product'); /*die()*/;
                }
            }else $msg="Vui lòng nhập đầy đủ thông tin"; 
